...
;12f675-PwmFet.mpas,63 :: 		begin
;12f675-PwmFet.mpas,64 :: 		if T0IF_bit=1 then begin
	BTFSS      T0IF_bit+0, BitPos(T0IF_bit+0)
	GOTO       L__Interrupt2
;12f675-PwmFet.mpas,66 :: 		if PWM_SIG=1 then begin
	BTFSS      GP0_bit+0, BitPos(GP0_bit+0)
	GOTO       L__Interrupt5
;12f675-PwmFet.mpas,67 :: 		ON_PWM:=VOL_PWM;
	MOVF       _VOL_PWM+0, 0
	MOVWF      _ON_PWM+0
;12f675-PwmFet.mpas,68 :: 		if ON_PWM=0 then
	MOVF       _VOL_PWM+0, 0
	XORLW      0
	BTFSS      STATUS+0, 2
	GOTO       L__Interrupt8
;12f675-PwmFet.mpas,69 :: 		TMR0:=ON_PWM
	...
	MOVWF      TMR0+0
	GOTO       L__Interrupt9
;12f675-PwmFet.mpas,70 :: 		else begin
L__Interrupt8:
;12f675-PwmFet.mpas,71 :: 		TMR0:=PWM_MAX-ON_PWM;
	MOVF       _ON_PWM+0, 0
	SUBLW      255
	MOVWF      TMR0+0
;12f675-PwmFet.mpas,72 :: 		PWM_SIG:=0;
	BCF        GP0_bit+0, BitPos(GP0_bit+0)
;12f675-PwmFet.mpas,73 :: 		end;
L__Interrupt9:
;12f675-PwmFet.mpas,74 :: 		end else begin
	GOTO       L__Interrupt6
L__Interrupt5:
;12f675-PwmFet.mpas,75 :: 		TMR0:=ON_PWM;
	MOVF       _ON_PWM+0, 0
	MOVWF      TMR0+0
;12f675-PwmFet.mpas,76 :: 		PWM_SIG:=1;
...
;12f675-PwmFet.mpas,91 :: 		begin
;12f675-PwmFet.mpas,92 :: 		if limit>0 then begin
	MOVF       FARG_Delay1ms_limit+0, 0
	SUBLW      0
	BTFSC      STATUS+0, 0
	GOTO       L__Delay1ms15
;12f675-PwmFet.mpas,93 :: 		ts:=TICK_1000;
	MOVF       _TICK_1000+0, 0
	MOVWF      R2+0
;12f675-PwmFet.mpas,94 :: 		if t<=ts then
	...
	BTFSS      STATUS+0, 0
	GOTO       L__Delay1ms18
;12f675-PwmFet.mpas,95 :: 		ts:=ts-t
	MOVF       FARG_Delay1ms_t+0, 0
	MOVWF      FSR
	MOVF       INDF+0, 0
	SUBWF      R2+0, 1
	GOTO       L__Delay1ms19
;12f675-PwmFet.mpas,96 :: 		else
L__Delay1ms18:
;12f675-PwmFet.mpas,97 :: 		ts:=255-t+1+ts;
	MOVF       FARG_Delay1ms_t+0, 0
	MOVWF      FSR
	MOVF       INDF+0, 0
	SUBLW      255
	...
	ADDWF      R2+0, 1
L__Delay1ms19:
;12f675-PwmFet.mpas,98 :: 		Result:=ts>=limit;
	MOVF       FARG_Delay1ms_limit+0, 0
	SUBWF      R2+0, 0
	MOVLW      255
	BTFSS      STATUS+0, 0
	MOVLW      0
	MOVWF      R1+0
;12f675-PwmFet.mpas,99 :: 		end else
	GOTO       L__Delay1ms16
L__Delay1ms15:
;12f675-PwmFet.mpas,100 :: 		Result:=True;
	MOVLW      255
	MOVWF      R1+0
L__Delay1ms16:
;12f675-PwmFet.mpas,101 :: 		if Result then
	MOVF       R1+0, 0
	BTFSC      STATUS+0, 2
	GOTO       L__Delay1ms21
;12f675-PwmFet.mpas,102 :: 		t:=TICK_1000;
	MOVF       FARG_Delay1ms_t+0, 0
	MOVWF      FSR
	MOVF       _TICK_1000+0, 0
	MOVWF      INDF+0
...
;12f675-PwmFet.mpas,106 :: 		CMCON:=7;
	MOVLW      7
	MOVWF      CMCON+0
;12f675-PwmFet.mpas,107 :: 		ANSEL:=%00000010;
	MOVLW      2
	MOVWF      ANSEL+0
...
;12f675-PwmFet.mpas,112 :: 		TRISIO5_bit:=0;  // Sig_VSE
...
;12f675-PwmFet.mpas,115 :: 		LED1_tm:=250;
	MOVLW      250
	MOVWF      _LED1_tm+0
;12f675-PwmFet.mpas,116 :: 		LED1_Toggle:=True;
	MOVLW      255
...
;12f675-PwmFet.mpas,118 :: 		VOL_PWM:=0;
	CLRF       _VOL_PWM+0
;12f675-PwmFet.mpas,119 :: 		out_short:=False;
	CLRF       _out_short+0
;12f675-PwmFet.mpas,120 :: 		short_count:=cShort_Recover*2;
	MOVLW      28
	MOVWF      _short_count+0
;12f675-PwmFet.mpas,121 :: 		short_check:=False;
	CLRF       _short_check+0
;12f675-PwmFet.mpas,122 :: 		TICK_1000:=0;
	CLRF       _TICK_1000+0
;12f675-PwmFet.mpas,123 :: 		LED_TMCount:=0;
	CLRF       _LED_TMCount+0
;12f675-PwmFet.mpas,125 :: 		OPTION_REG:=%01010000;        // ~2KHz @ 4MHz, enable weak pull-up
	MOVLW      80
	MOVWF      OPTION_REG+0
;12f675-PwmFet.mpas,127 :: 		WPU4_bit:=1;
	BSF        WPU4_bit+0, BitPos(WPU4_bit+0)
;12f675-PwmFet.mpas,129 :: 		TMR0IE_bit:=1;
...
;12f675-PwmFet.mpas,134 :: 		TMR1H:=TMR1H_LOAD;
	MOVLW      252
	MOVWF      TMR1H+0
;12f675-PwmFet.mpas,135 :: 		TMR1ON_bit:=1;
	BSF        TMR1ON_bit+0, BitPos(TMR1ON_bit+0)
;12f675-PwmFet.mpas,136 :: 		GIE_bit:=1;      // enable Interrupt
	BSF        GIE_bit+0, BitPos(GIE_bit+0)
;12f675-PwmFet.mpas,138 :: 		LED1_Timer:=TICK_1000;
	CLRF       _LED1_Timer+0
;12f675-PwmFet.mpas,139 :: 		ADC_Timer:=TICK_1000;
	CLRF       _ADC_Timer+0
;12f675-PwmFet.mpas,140 :: 		short_Timer:=TICK_1000;
	CLRF       _short_Timer+0
;12f675-PwmFet.mpas,142 :: 		Delay_10ms;
	CALL       _Delay_10ms+0
;12f675-PwmFet.mpas,144 :: 		if VOut5=0 then begin
	BTFSC      GP3_bit+0, BitPos(GP3_bit+0)
	GOTO       L__main25
;12f675-PwmFet.mpas,145 :: 		vTarget:=c051;
	MOVLW      92
	MOVWF      _vTarget+0
	MOVLW      1
	MOVWF      _vTarget+1
;12f675-PwmFet.mpas,146 :: 		VSE_LO:=c043;
	MOVLW      41
	MOVWF      _VSE_LO+0
	MOVLW      1
	MOVWF      _VSE_LO+1
...
;12f675-PwmFet.mpas,148 :: 		vTarget:=c138;
	...
	MOVLW      3
	MOVWF      _vTarget+1
;12f675-PwmFet.mpas,149 :: 		VSE_LO:=c118;
	MOVLW      37
	MOVWF      _VSE_LO+0
	MOVLW      3
	MOVWF      _VSE_LO+1
;12f675-PwmFet.mpas,150 :: 		end;
L__main26:
;12f675-PwmFet.mpas,152 :: 		while True do begin
L__main28:
;12f675-PwmFet.mpas,153 :: 		adc_vol:=ADC_Read(1);
	MOVLW      1
	...
	MOVF       R0+1, 0
	MOVWF      _adc_vol+1
;12f675-PwmFet.mpas,155 :: 		if adc_vol>=VSE_LO then
	MOVF       _VSE_LO+1, 0
	SUBWF      R0+1, 0
	BTFSS      STATUS+0, 2
	GOTO       L__main90
...
L__main90:
	BTFSS      STATUS+0, 0
	GOTO       L__main33
;12f675-PwmFet.mpas,156 :: 		Sig_VSE:=1
	BSF        GP5_bit+0, BitPos(GP5_bit+0)
	GOTO       L__main34
;12f675-PwmFet.mpas,157 :: 		else
L__main33:
;12f675-PwmFet.mpas,158 :: 		Sig_VSE:=0;
	BCF        GP5_bit+0, BitPos(GP5_bit+0)
L__main34:
;12f675-PwmFet.mpas,160 :: 		if (not out_short) and Delay1ms(ADC_Timer,1) then begin
	COMF       _out_short+0, 0
	MOVWF      FLOC__main+0
	MOVLW      _ADC_Timer+0
...
L__main93:
	BTFSC      STATUS+0, 0
	GOTO       L__main48
;12f675-PwmFet.mpas,171 :: 		if VOL_PWM>0 then
	MOVF       _VOL_PWM+0, 0
	SUBLW      0
	BTFSC      STATUS+0, 0
	GOTO       L__main51
