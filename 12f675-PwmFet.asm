...
;12f675-PwmFet.mpas,57 :: 		if T0IF_bit=1 then begin
	...
	GOTO       L__Interrupt2
;12f675-PwmFet.mpas,58 :: 		PWM_FLAG:=not PWM_FLAG;
	MOVLW
	XORWF      _PWM_FLAG+0, 1
...
;12f675-PwmFet.mpas,129 :: 		T1CKPS0_bit:=1;               // timer prescaler 1:2
...
;12f675-PwmFet.mpas,156 :: 		Read_ADC;
	CALL       _Read_ADC+0
;12f675-PwmFet.mpas,157 :: 		if (adc_vol>vTarget) and (((not Inc_Flag) and (adc_vol-vTarget>=18)) or Inc_Flag) then begin
	MOVF       _adc_vol+1, 0
	SUBWF      _vTarget+1, 0
	BTFSS      STATUS+0, 2
	GOTO       L__main62
...
;12f675-PwmFet.mpas,160 :: 		Dec(VOL_PWM);
	DECF       _VOL_PWM+0, 1
L__main41:
;12f675-PwmFet.mpas,161 :: 		end else if (adc_vol<=vTarget) and ((Inc_Flag and (vTarget-adc_vol>=18)) or (not Inc_Flag)) then begin
	GOTO       L__main39
L__main38:
	MOVF       _adc_vol+1, 0
	SUBWF      _vTarget+1, 0
...
L__main69:
	...
	BTFSC      STATUS+0, 2
	GOTO       L__main44
;12f675-PwmFet.mpas,162 :: 		Inc_Flag:=1;
	BSF        _Inc_Flag+0, BitPos(_Inc_Flag+0)
;12f675-PwmFet.mpas,163 :: 		if VOL_PWM<PWM_MAX then
	MOVLW      255
	SUBWF      _VOL_PWM+0, 0
	BTFSC      STATUS+0, 0
	GOTO       L__main47
;12f675-PwmFet.mpas,164 :: 		Inc(VOL_PWM);
	INCF       _VOL_PWM+0, 1
L__main47:
;12f675-PwmFet.mpas,165 :: 		end;
L__main44:
L__main39:
;12f675-PwmFet.mpas,166 :: 		if PWM_FLAG then
	BTFSS      _PWM_FLAG+0, BitPos(_PWM_FLAG+0)
	GOTO       L__main50
;12f675-PwmFet.mpas,167 :: 		continue;
	GOTO       L__main33
L__main50:
;12f675-PwmFet.mpas,169 :: 		if (Hi(adc_vol)=0) and (Lo(adc_vol)<=cLow) then
	MOVF       _adc_vol+1, 0
	XORLW      0
	MOVLW      255
	BTFSS      STATUS+0, 2
	...
	BTFSC      STATUS+0, 2
	GOTO       L__main53
;12f675-PwmFet.mpas,170 :: 		LED1_tm:=64
	MOVLW      64
	MOVWF      _LED1_tm+0
	GOTO       L__main54
;12f675-PwmFet.mpas,171 :: 		else if VOL_PWM=PWM_MAX then
L__main53:
	MOVF       _VOL_PWM+0, 0
	XORLW      255
	BTFSS      STATUS+0, 2
	GOTO       L__main56
;12f675-PwmFet.mpas,172 :: 		LED1_tm:=128
	MOVLW      128
	MOVWF      _LED1_tm+0
	GOTO       L__main57
;12f675-PwmFet.mpas,173 :: 		else
L__main56:
;12f675-PwmFet.mpas,174 :: 		LED1_tm:=240;
	MOVLW      240
	MOVWF      _LED1_tm+0
L__main57:
L__main54:
